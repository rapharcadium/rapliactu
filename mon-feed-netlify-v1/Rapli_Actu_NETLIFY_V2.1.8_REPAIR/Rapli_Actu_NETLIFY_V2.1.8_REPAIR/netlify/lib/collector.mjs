...
import { parseRss, parseGenericHtml, parseFFF, parseChroniquesBleues, parseLeMarinCandidates, parseLeMarinArticlePage, parseMarkdownLinks, parseSitemap, sitemapItems } from './parsers.mjs';
...
function normalize(source,item,index=0,mode='rss',seen={}){
  ...
  return {id,sourceId:source.id,source:source.name,domain:source.domain,category:source.category,icon:source.icon,color:source.color,
    title:analysis.cleanTitle,originalTitle,summary,link,publishedAt,dateInferred:inferred,sourceRevision:source.revision||null,discoveredAt,
    ...
    availability:sourceDefaultAvailability(source),media:inferMedia(originalTitle,summary),enrichedAt:null};
}
...
async function viaRss2Json(rssUrl){
  ...
  const items=data.items.map(x=>({title:x.title||'',summary:x.description||x.content||'',link:x.link||x.guid||'',publishedAt:x.pubDate?new Date(x.pubDate).toISOString():null})).filter(x=>x.title&&x.link);
...
async function viaLeMarin(source){
  if(!source.page)throw new Error('Aucune page Le Marin');
  const home=await fetchText(source.page,{timeoutMs:8500});
  const candidates=parseLeMarinCandidates(home.text,source).slice(0,24);
  if(!candidates.length)throw new Error('Le Marin: 0 lien article détecté');
  const hydrated=[];
  const CONCURRENCY=8;
  for(let i=0;i<candidates.length;i+=CONCURRENCY){
    const group=candidates.slice(i,i+CONCURRENCY);
    const batch=await Promise.allSettled(group.map(async item=>{
      const page=await fetchText(item.link,{timeoutMs:4500});
      const meta=parseLeMarinArticlePage(page.text,item.link);
      const publishedAt=meta.publishedAt||item.publishedAt;
      if(!publishedAt)throw new Error('date absente');
      const ts=Date.parse(publishedAt);
      if(!Number.isFinite(ts))throw new Error('date invalide');
      // Une vraie home d'actualité ne doit pas réinjecter d'archives très anciennes.
      if(ts < Date.now()-120*86400000)throw new Error('article trop ancien');
      return {title:meta.title||item.title,summary:meta.summary||item.summary||'',link:meta.link||item.link,publishedAt:new Date(ts).toISOString()};
    }));
    for(const r of batch)if(r.status==='fulfilled')hydrated.push(r.value);
    // Dès qu'on possède assez d'articles valides, on peut éviter de marteler le site.
    if(hydrated.length>=14)break;
  }
  const byUrl=new Map();
  for(const item of hydrated){if(!item.link||!item.title)continue;const old=byUrl.get(item.link);if(!old||Date.parse(item.publishedAt)>Date.parse(old.publishedAt||0))byUrl.set(item.link,item)}
  const items=[...byUrl.values()].sort((a,b)=>Date.parse(b.publishedAt)-Date.parse(a.publishedAt));
  if(!items.length)throw new Error('Le Marin: aucun article daté et accessible');
  return{mode:'lemarin-direct',items,endpoint:source.page};
}

async function viaHtml(source){if(!source.page)throw new Error('Aucune page directe');const{text}=await fetchText(source.page,{timeoutMs:8500});let items;if(source.adapter==='fff')items=parseFFF(text,source);else if(source.adapter==='chroniquesBleues')items=parseChroniquesBleues(text,source);else if(source.adapter==='lemarin')throw new Error('Le Marin doit utiliser viaLeMarin');else items=parseGenericHtml(text,source);if(!items.length)throw new Error('0 article détecté sur la page');return{mode:'html',items,endpoint:source.page}}
async function fetchSource(source){
  ...
  if(source.adapter==='lemarin'){try{return await viaLeMarin(source)}catch(e){errors.push(`LEMARIN: ${e.message}`)}}
  else if(source.page&&source.adapter!=='sitemap'){try{return await viaHtml(source)}catch(e){errors.push(`HTML: ${e.message}`)}}
...
export async function collectAll(){
  ...
  const feed={version:'2.1.8',generatedAt:nowIso(),cycleStartedAt,currentPerSource:CURRENT_PER_SOURCE,historyDays:HISTORY_DAYS,stats:{sources:sources.length,ok,stale,errors,disabled,articles:articles.length,newThisCycle:discovered.length},sources,articles};
