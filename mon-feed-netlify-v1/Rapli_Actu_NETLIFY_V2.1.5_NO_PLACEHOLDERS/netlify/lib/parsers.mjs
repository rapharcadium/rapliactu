...
function imageFromDescription(html = '', baseUrl = '') {
  ...
  return m ? sanitizeImageUrl(canonicalUrl(absoluteUrl(m[1], baseUrl))) : '';
}
function rssImage(item, baseUrl) {
  ...
  if (mediaUrl) return sanitizeImageUrl(canonicalUrl(absoluteUrl(mediaUrl, baseUrl)));
  const enc = item?.enclosure;
  if (enc?.['@_url'] && (!enc?.['@_type'] || String(enc['@_type']).startsWith('image/'))) return sanitizeImageUrl(canonicalUrl(absoluteUrl(enc['@_url'], baseUrl)));
...
function imageFromBox($, box, base) {
  ...
  return raw ? sanitizeImageUrl(canonicalUrl(absoluteUrl(raw, base))) : '';
}
...
export function parseArticleMeta(text, url) {
  ...
  const image=sanitizeImageUrl(canonicalUrl(absoluteUrl(meta('og:image',true)||meta('twitter:image')||$('article img').first().attr('src')||'',url)));
