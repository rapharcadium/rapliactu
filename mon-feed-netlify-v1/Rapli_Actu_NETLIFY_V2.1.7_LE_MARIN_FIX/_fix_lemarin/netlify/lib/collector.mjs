...
import { parseRss, parseGenericHtml, parseFFF, parseChroniquesBleues, parseLeMarin, parseMarkdownLinks, parseSitemap, sitemapItems } from './parsers.mjs';
...
async function viaHtml(source){if(!source.page)throw new Error('Aucune page directe');const{text}=await fetchText(source.page,{timeoutMs:8500});let items;if(source.adapter==='fff')items=parseFFF(text,source);else if(source.adapter==='chroniquesBleues')items=parseChroniquesBleues(text,source);else if(source.adapter==='lemarin')items=parseLeMarin(text,source);else items=parseGenericHtml(text,source);if(!items.length)throw new Error('0 article détecté sur la page');return{mode:'html',items,endpoint:source.page}}
...
export async function collectAll(){
  ...
  const feed={version:'2.1.7',generatedAt:nowIso(),cycleStartedAt,currentPerSource:CURRENT_PER_SOURCE,historyDays:HISTORY_DAYS,stats:{sources:sources.length,ok,stale,errors,disabled,articles:articles.length,newThisCycle:discovered.length},sources,articles};
